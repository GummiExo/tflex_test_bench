%function [td ts tr tp initial_value final_value peak_value] = step_parameters(input1 input2 output)
clc; close all;
...
t1 = input1.Timestamp(t1_pos_input1);
t1_pos_output = find(output.Timestamp > t1,1);
...
tf1 = input1.Timestamp(tf1_pos_input1);
tf1_pos_output = find(output.Timestamp > tf1,1) - 1;
tf1_pos_input2 = find(input2.Timestamp > tf1,1) - 1;

%Final and Initial Values of Torque
initial_value1 = output.Data(t1_pos_output);
final_value1 = output.Data(tf1_pos_output);
...
stabilization_pos1 = intersect(find(output.filtered(min_peak_pos_output:tf1_pos_output) >= (final_value1+abs(min_peak_value) - window_stabilization1)),find(output.filtered(min_peak_pos_output:tf1_pos_output) <= (final_value1+abs(min_peak_value) + window_stabilization1)));
...
t2 = input2.Timestamp(t2_pos_input2);
t2_pos_output = find(output.Timestamp > t2,1) - 1;
...
tf2 = input2.Timestamp(tf2_pos_input2(1));
tf2_pos_output = find(output.Timestamp > tf2,1);

%Final and Initial Values of Torque
initial_value2 = output.Data(t2_pos_output);
final_value2 = output.Data(tf2_pos_output);
...
%% Plot
 ...
 %Stabilization time
 plot(output.Timestamp(find(output.Timestamp == ts1,1)),output.Timestamp(find(output.Timestamp == ts1,1)),'*');
 %Rise time
 plot(output.Timestamp(find(output.Timestamp == tr1,1)),output.Timestamp(find(output.Timestamp == tr1,1)),'*');
 %Peaks 
 plot(output.Timestamp(find(output.Timestamp == max_peak_pos_output,1)),output.Timestamp(find(output.Timestamp == max_peak_pos_output,1)),'*');
 plot(output.Timestamp(find(output.Timestamp == min_peak_pos_output,1)),output.Timestamp(find(output.Timestamp == min_peak_pos_output,1)),'*');
 
 legend('Goal Frontal','Goal Posterior','Torque','ts','tr','max peak','min peak')
